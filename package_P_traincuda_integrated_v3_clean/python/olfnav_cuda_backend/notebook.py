...
"""Notebook-friendly CUDA backend adapter for olfactory_navigation FSVI.
...
switch to the custom CUDA backup backend without overloading the original
``train`` method:
...
Package K/K7/K8/K9: full-pipeline FSVI with CUDA backup replacement, not a raw
...
class CudaNotebookTrainResult:
    ...
    def to_dataframe(self):
        """Return per-iteration rows as a pandas DataFrame."""
        import pandas as pd
        return pd.DataFrame(self.rows)

    def __repr__(self) -> str:
        completed = self.summary.get("completed_expansions", len(self.rows))
        requested = self.summary.get("requested_expansions", completed)
        alpha = self.summary.get("last_alpha_after")
        belief = self.summary.get("last_belief_total_after")
        wall = self.summary.get("total_wall_s")
        return (
            f"CudaNotebookTrainResult(completed={completed}/{requested}, "
            f"alpha={alpha}, belief={belief}, wall_s={wall})"
        )


def _show_cuda_training_report(result: Any, *, display_rows: int = 10) -> None:
    """Render a compact traincuda report.

    This local helper intentionally lives in the backend module because
    ``CudaFSVI_Agent.traincuda(..., visual=True)`` calls it directly.  Keeping it
    here avoids a fragile cross-package dependency from ``olfnav_cuda_backend``
    back to ``olfnav_cuda_notebook.visual``.
    """
...
def package_root() -> Path:
    # .../package_K_inprocess_cuda_backend/python/olfnav_cuda_backend/notebook.py
...
def _normalize_non_layered_metadata(obj: Any) -> Any:
    """Normalize non-layered olfactory metadata in-place.

    Some reconstructed environments store layer metadata as the boolean False.
    Upstream SimulationHistory expects None for non-layered environments; False
    is truthy for its "has layer metadata" branch and later breaks add_step()
    or plot() with shape/bool errors.
    """
    if obj is None:
        return None
    for attr in (
...
def normalize_agent_environment(agent: Any) -> Any:
    """Best-effort normalization of an agent and its environment."""
    _normalize_non_layered_metadata(agent)
    try:
        _normalize_non_layered_metadata(agent.environment)
    except Exception:
        pass
    try:
        _normalize_non_layered_metadata(agent._agent.environment)
    except Exception:
        pass
    return agent
...
class CudaFSVI_Agent:
    ...
    ``ag_cuda.traincuda(...)`` uses the custom CUDA backend. This avoids
    ...
    def __init__(
        ...
        *,
        ...
        self._cuda_backend_config = {
            "lib_path": str(self.cuda_lib_path),
            "device": self.cuda_device,
            "version": self.cuda_version,
            "gamma": self.gamma,
            "max_belief_growth": self.max_belief_growth,
            "prune_interval": self.prune_interval,
            "prune_level": self.prune_level,
            "compact_cuda_backup_before_append": self.compact_cuda_backup_before_append,
            "cuda_backup_compact_prune_level": self.cuda_backup_compact_prune_level,
            "mdp_use_gpu": self.mdp_use_gpu,
        }
    ...
    def traincuda(
        ...
        **unused_upstream_kwargs: Any,
        ...
        """
        normalize_agent_environment(self._agent)
        cfg = self._make_cfg(
            gamma=gamma,
        ...
        try:
            ...
            if visual:
                _show_cuda_training_report(result, display_rows=int(display_rows))
