...
static constexpr int BLOCK_V7 = 256;

__global__ void kernel_build_GAO_col_all(
    double* A_col,
    const double* Gamma,
    const double* O,
    const double* T,
    ...
    int nS)
{
    ...
    const long long total = K * nS;
    if (idx >= total) return;

    const int s = static_cast<int>(idx / K);
    ...
    A_col[row + static_cast<long long>(s) * K] = acc;
}
    ...
    int nG)
{
    ...
    for (int i = tid; i < nG; i += blockDim.x) {
        ...
        const double v = Y[row + static_cast<long long>(j) * K];
        if (v > best_val) {
            best_val = v;
            best_i = i;
        }
    ...
    __shared__ double sh_val[BLOCK_V7];
    __shared__ int sh_idx[BLOCK_V7];
    sh_val[tid] = best_val;
    sh_idx[tid] = best_i;
    __syncthreads();

    for (int stride = BLOCK_V7 / 2; stride > 0; stride >>= 1) {
...
__global__ void kernel_generation_from_GAO_all(
    double* BKP,
    double* V_new,
    const int* g_star_ao,
    const double* B,
    ...
    int nG)
{
    ...
    for (int a = 0; a < nA; ++a) {
        ...
        for (int s = threadIdx.x; s < nS; s += blockDim.x) {
            ...
            for (int o = 0; o < nO; ++o) {
                ...
                const long long row = (static_cast<long long>(a) * nO + o) * nG + bi;
                cont += GAO[row + static_cast<long long>(s) * K];
            }
    ...
    for (int s = threadIdx.x; s < nS; s += blockDim.x) {
        ...
        for (int o = 0; o < nO; ++o) {
            ...
            const long long row = (static_cast<long long>(a_sel) * nO + o) * nG + bi;
            cont += GAO[row + static_cast<long long>(s) * K];
        }
...
struct GpuContextV7ReuseGAO {
    const Problem& p;
    const int nG0;
    const int nGmax;
    const long long Kmax;
    ...
    explicit GpuContextV7ReuseGAO(const Problem& prob)
        : p(prob),
          nG0(static_cast<int>(prob.Gamma.size() / static_cast<size_t>(prob.nS))),
          nGmax(std::max(nG0, prob.nB)),
          Kmax(static_cast<long long>(prob.nA) * prob.nO * std::max(nG0, prob.nB))
    {
        ...
        CHECK_CUDA(cudaMalloc(&d_GAO, static_cast<size_t>(Kmax) * p.nS * sizeof(double)));
        CHECK_CUDA(cudaMalloc(&d_Y, static_cast<size_t>(Kmax) * p.nB * sizeof(double)));
    ...
    void iterate()
    {
        ...
        const long long K = static_cast<long long>(p.nA) * p.nO * nG_cur;
        const long long totalGAO = K * p.nS;
        const double alpha = 1.0;
        const double beta = 0.0;

        kernel_build_GAO_col_all<<<static_cast<int>((totalGAO + threads - 1) / threads), threads>>>(
        ...
        kernel_argmax_from_Y_all<<<dim3(p.nB, p.nA, p.nO), threads>>>(
            d_gstar, d_Y, p.nB, p.nA, p.nO, nG_cur);
        CHECK_CUDA(cudaGetLastError());

        kernel_generation_from_GAO_all<<<p.nB, threads, smem_gen>>>(
            d_GammaNext,
            d_VnewNext,
            d_gstar,
            d_B,
...
int main(int argc, char** argv)
{
    try {
        ...
        GpuContextV7ReuseGAO ctx(p);

        BenchSummary summary;
        summary.version = "v7_bench_transition_correct_reuse_gao";
